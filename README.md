...
## Dinamismo vial (animaciones)

Para dar sensación de movimiento al mapa (estilo Waze/Uber), se añadieron tres animaciones
puramente visuales, sin depender de ninguna fuente de datos nueva:

- **Flujo animado en vías principales**: las líneas de `vias_principales.geojson` usan un
  patrón de guiones (`stroke-dasharray`) que se desplaza continuamente vía CSS
  (`@keyframes viaFlow`), dando el efecto de "flujo" que usan Google Maps/Waze sobre las
  rutas. Se puede desactivar con el checkbox **"Animar flujo en vías principales"**.
- **Tránsito animado (simulación)**: una muestra de ~22 tramos de `vias_principales.geojson`
  (repartidos por todo el radio, no solo los más largos) recibe un marcador en forma de
  flecha que se desplaza de ida y vuelta a velocidad e inicio aleatorios
  (`requestAnimationFrame`, con orientación calculada por rumbo entre vértices). Es una
  **simulación ilustrativa**: no representa vehículos, rutas ni horarios reales de
  transporte público (ver "Nota Técnica"). Se pausa automáticamente si la pestaña no está
  visible y se puede apagar con su propio checkbox.
- **Pulso tipo radar** en el marcador de la U.E. Ángel Modesto Paredes: anillos concéntricos
  con `animation: pulseRing` que laten alrededor del pin de referencia.

Las tres animaciones respetan `prefers-reduced-motion: reduce` (se desactivan si el usuario
tiene esa preferencia de accesibilidad activada en su sistema).
